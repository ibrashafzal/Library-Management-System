...
namespace LMS.Reports
{
    ...
    {
        public static void ExportIssueToPDF(
          List<Issue> issuesbook,
           string pdfPath,
           string reportTitle,
           DateTime startDate,
           DateTime endDate,
        ...
        {
            if (issuesbook == null || issuesbook.Count == 0)
            {
            ...
            try
            {
                ...
                // Headers
                string[] headers = { "ID", "Student Id", "Book Id", "Quantity" };
                ...
                // Rows
                foreach (var issue in issuesbook)
                {
                    table.AddCell(new Phrase(issue.Id.ToString(), FontFactory.GetFont(FontFactory.HELVETICA, 9)));
                    table.AddCell(new Phrase(issue.StudentId.ToString(), FontFactory.GetFont(FontFactory.HELVETICA, 9)));
                    table.AddCell(new Phrase(issue.BookId.ToString(), FontFactory.GetFont(FontFactory.HELVETICA, 9)));
                    table.AddCell(new Phrase(issue.IssueQuantity.ToString(), FontFactory.GetFont(FontFactory.HELVETICA, 9)));
